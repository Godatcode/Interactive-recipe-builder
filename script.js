...
cookingPot.addEventListener("drop", (event) => {
  ...
  if (
    ingredient &&
    !Array.from(cookingPot.children).some(
      (child) => child.dataset.name === ingredient
    )
  ) {
...
document
  .getElementById("fetch-recipes-btn")
  .addEventListener("click", async () => {
    const selectedIngredients = Array.from(cookingPot.children)
      .map((div) => div.dataset.name)
      .join(",");
    if (!selectedIngredients) {
      recipesContainer.innerHTML =
        "<p>Please add ingredients to the cooking pot first! 🍲</p>";
      return;
    }
    ...
    try {
      ...
      if (recipes.length === 0) {
        recipesContainer.innerHTML =
          "<p>No recipes found for the selected ingredients. 😔</p><img src='https://media.giphy.com/media/1Xc0K8DcnOZMG/giphy.gif' alt='no recipe found' />";
      } else {
...
function displayYouTubeVideos(videos) {
  ...
  if (videos.length > 0) {
    ...
    videosContainer.innerHTML =
      "<p>No videos found for the selected ingredients. 😞</p>";
  }
}
document
